...
> 리서치 에이전트는 `general-purpose` 빌트인 타입을 사용하되, 반드시 `.ai/agents/{name}.md` 파일로 정의한다. 파일에는 역할·조사 범위·팀 통신 프로토콜을 명시하여 재사용성과 협업 품질을 보장한다.
...
### 에이전트 정의 파일
위치: `프로젝트/.ai/agents/{agent-name}.md`
필수 섹션: 핵심 역할, 작업 원칙, 입력/출력 프로토콜, 에러 핸들링, 협업
팀 모드 추가 섹션: **팀 통신 프로토콜** (메시지 수신/발신, 작업 요청 범위)

### 스킬 파일 구조
위치: `프로젝트/.ai/skills/{skill-name}/SKILL.md` (프로젝트 레벨)
또는: `~/.ai/skills/{skill-name}/SKILL.md` (글로벌 레벨)
