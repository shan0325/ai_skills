...
**핵심 원칙:**
1. 에이전트 정의(`.ai/agents/`)와 스킬(`.ai/skills/`)을 생성한다.
2. **에이전트 팀을 기본 실행 모드로 사용한다.**
3. **AI_INSTRUCTIONS.md에 하네스 포인터를 등록한다.** — 새 세션에서 오케스트레이터 스킬이 트리거되도록 최소한의 포인터(트리거 규칙 + 변경 이력)만 기록한다.
4. **하네스는 고정물이 아니라 진화하는 시스템이다.** — 매 실행 후 피드백을 반영하고, 에이전트·스킬·AI_INSTRUCTIONS.md를 지속 갱신한다.
...
1. `프로젝트/.ai/agents/`, `프로젝트/.ai/skills/`, `프로젝트/AI_INSTRUCTIONS.md`를 읽는다
...
3. 기존 에이전트/스킬 목록과 AI_INSTRUCTIONS.md 기록을 대조하여 불일치(drift)를 감지한다
...
신규 에이전트 생성 전, `프로젝트/.ai/agents/`의 기존 에이전트와 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 역할이 겹치는 에이전트가 다른 이름으로 누적되기 쉽다.

> 중복 분류 기준과 재사용 설계는 `references/agent-design-patterns.md`의 "에이전트 재사용 설계" 참조.

**모든 에이전트는 반드시 `프로젝트/.ai/agents/{name}.md` 파일로 정의한다.** 에이전트 정의 파일 없이 Agent 도구의 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
...
각 에이전트를 `프로젝트/.ai/agents/{name}.md`에 정의한다. 필수 섹션: 핵심 역할, 작업 원칙, 입력/출력 프로토콜, 에러 핸들링, 협업. 에이전트 팀 모드에서는 `## 팀 통신 프로토콜` 섹션을 추가하여 메시지 수신/발신 대상과 작업 요청 범위를 명시한다.
...
각 에이전트가 사용할 스킬을 `프로젝트/.ai/skills/{name}/SKILL.md`에 생성한다. 상세 작성 가이드는 `references/skill-writing-guide.md` 참조.

#### 4-0. 기존 스킬 중복 검토

신규 스킬 생성 전, `프로젝트/.ai/skills/`의 기존 스킬과 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 기능이 겹치는 스킬이 다른 이름으로 누적되기 쉽다.
...
description은 스킬의 유일한 트리거 메커니즘이다. AI Agent는 트리거를 보수적으로 판단하는 경향이 있으므로, description을 **적극적("pushy")**으로 작성한다.
...
#### 5-4. AI_INSTRUCTIONS.md 하네스 포인터 등록

하네스 구성 완료 후, 프로젝트의 `AI_INSTRUCTIONS.md`에 최소한의 포인터를 등록한다. AI_INSTRUCTIONS.md는 새 세션마다 로딩되므로, 하네스 존재와 트리거 규칙만 기록하면 오케스트레이터 스킬이 나머지를 처리한다.

**AI_INSTRUCTIONS.md 템플릿:**
...
**AI_INSTRUCTIONS.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.ai/agents/`, `.ai/skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. AI_INSTRUCTIONS.md는 **포인터(트리거 규칙) + 변경 이력**만 담는다.
...
모든 변경은 AI_INSTRUCTIONS.md의 **변경 이력** 테이블에 기록한다 (Phase 5-4 템플릿의 "변경 이력" 섹션과 동일 테이블):
...
**Step 1: 현황 감사**
- `.ai/agents/` 파일 목록과 오케스트레이터 스킬의 에이전트 구성 비교 → 불일치 목록 생성
- `.ai/skills/` 디렉토리 목록과 오케스트레이터 스킬의 스킬 구성 비교 → 불일치 목록 생성
- 감사 결과를 사용자에게 보고한다
...
**Step 3: AI_INSTRUCTIONS.md 변경 이력 갱신**
...
- AI_INSTRUCTIONS.md와 실제 파일의 일치 여부 최종 확인

## 산출물 체크리스트

생성 완료 후 확인:

- [ ] `프로젝트/.ai/agents/` — **에이전트 정의 파일 필수 생성** (빌트인 타입이라도 파일 생성 필수)
- [ ] `프로젝트/.ai/skills/` — 스킬 파일들 (SKILL.md + references/)
...
- [ ] **AI_INSTRUCTIONS.md에 하네스 포인터 등록** (트리거 규칙 + 변경 이력)
- [ ] **AI_INSTRUCTIONS.md 변경 이력에 에이전트/스킬 추가/삭제/수정 기록**
